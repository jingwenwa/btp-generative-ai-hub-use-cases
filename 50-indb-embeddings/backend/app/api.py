...
def update_categories_and_projects():
    ...
    for index, (title, description) in enumerate(categories.items()):
        insert_sql = f"""
            INSERT INTO CATEGORIES ("CATEGORY_ID", "CATEGORY_LABEL", "CATEGORY_DESCR")
...
def get_categories():
    sql_query = """
        SELECT "CATEGORY_ID", "CATEGORY_LABEL", "CATEGORY_DESCR"
        FROM "CATEGORIES"
    """
    hana_df = dataframe.DataFrame(connection, sql_query)
...
def get_clusters():
    ...
    hana_df = dataframe.DataFrame(connection, sql_query)
    clusters = hana_df.collect()
    
    formatted_clusters = [
        {"x": row["x"], "y": row["y"], "CLUSTER_ID": row["CLUSTER_ID"], "PROJECT_NUMBER": row["PROJECT_NUMBER"]}
...
def get_clusters_description():
    ...
    formatted_cluster_description = [
        {"CLUSTER_ID": row["CLUSTER_ID"], "CLUSTER_DESCRIPTION": row["CLUSTER_DESCRIPTION"]}
        for _, row in clusters.iterrows()
    ]
...
def get_projects_by_architect_and_cluster():
    ...
    """
    if expert:
        sql_query += f' WHERE a."architect" = \'{expert.replace("\'", "\'\'")}\''
    sql_query += " GROUP BY a.\"architect\", c.\"CLUSTER_ID\""
...
def insert_text_and_vector():
    ...
    table_name = data.get('table_name', 'TCM_SAMPLE')
    text = data.get('text')
    
    create_table_if_not_exists(schema_name, table_name)
    
    sql_insert = f"INSERT INTO {schema_name}.{table_name} (TEXT) SELECT '{text}' FROM DUMMY"
    cursor = connection.connection.cursor()
